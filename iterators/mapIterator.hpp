...
/*   Updated: 2022/06/23 18:02:10 by anasr            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
namespace ft{
	...
	public:
		...
		// 	mapIterator temp(it_start->getRoot(););
		// 	difference_type count = 0
		// 	for (; rhs.it_start != temp.it_start; ++count)
		// 		temp = getNextMinimum(temp);
	...
	template <typename T>
	mapIterator<T> operator+(typename mapIterator<T>::difference_type n, const mapIterator<T>& it){
		return mapIterator<T>(it + n);
	}
